...
def register():
    ...
    if form.validate_on_submit():
        ...
        new = User(username= form.username.data,email=form.email.data, password=form.password.data, usertype= 'parent')
        db.session.add(new)
        db.session.commit()
...
def login():
    form = LoginForm()
    if form.validate_on_submit():
        user = User.query.filter_by(email=form.email.data,password=form.password.data, usertype= 'parent' ).first()
        user1 = User.query.filter_by(email=form.email.data, usertype= 'doctor').first()
        user2 = User.query.filter_by(email=form.email.data,password=form.password.data, usertype= 'admin').first()
        if user:
...
@app.route('/dbukconfirm/<int:id>', methods=['GET', 'POST'])
@login_required
def dbukconfirm(id):
    task = BookDoctor.query.get_or_404(id)
    if request.method=="POST":
        ...
        db.session.commit()
        return redirect('/dbookingconfirm/'+str(task.id))
    else:
        return render_template("dbukconfirm.html")
